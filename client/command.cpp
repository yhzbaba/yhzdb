...
int ICommand::recvReply(ossSocket &sock)
{
  ...
  // then, get the value of the length
  length = *(int *)_recvBuf;
...
int ConnectCommand::execute(ossSocket &sock, std::vector<std::string> &argVec)
{
  int ret = EDB_OK;
  sock.close();
  sock.setAddress(argVec[0].c_str(), atoi(argVec[1].c_str()));
  ret = sock.initSocket();
  if (ret)
  {
...
int ConnectCommand::handleReply()
{
  int ret = EDB_OK;
  return ret;
}
...
int HelpCommand::execute(ossSocket &sock, std::vector<std::string> &argVec)
{
  ...
  printf("%s [number] -- sending a tet command to emeralddb server\n", COMMAND_TEST);
