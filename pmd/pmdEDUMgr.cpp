 ...
 * ==================================================================
 * 
 * Filename: pmdEDUMgr.cpp
 * 
 * Created: 11/22/2020 15:56 PM
 * 
 * Author: Yuan Haizhuo
 * 
 * thread pool
...
// creating edu status from running to waiting
...
int pmdEDUMgr::startEDU(EDU_TYPES type, void *arg, EDUID *eduid)
{
  ...
  // switch agent type for the EDU(call diffrent agent entry point)
  eduCB->setType(type);
  eduCB->setStatus(PMD_EDU_WAITING);
  _runQueue[eduID] = eduCB;
  *eduid = eduID;
...
// 从 waiting / creating 决定销毁还是放入线程池
